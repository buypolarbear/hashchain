...
class MultichainClient
{
     ...
     */
    public function __construct($url, $username, $password, $timeout = 5)
    {
     ...
     * @return mixed
     */
    public function grantFrom($fromAddress, $toAddresses, $permissions, $nativeAmount = 0, $startBlock = 0, $endBlock = -1, $comment = '', $commentTo = '')
    {
     ...
     * @return mixed
     */
    public function listAddressTransactions($address, $count = 50, $skip = 0, $verbose = false)
    {
    ...
    /**
     * Returns metadata of an unspent transaction output vout of txid. For a MultiChain blockchain, includes assets
     * and permissions fields listing any assets or permission changes encoded within the output. Set confirmed to true
     * to include unconfirmed transaction outputs.
     *
     * @param $txId
    ...
    {
        return $this->jsonRPCClient->execute("gettxoutdata", array($txId, $vOut));
    }
    ...
    {
        return $this->jsonRPCClient->execute("signmessage", array($address, $message));
    }


    public function verifyMessage($address, $signature, $message)
    {
        return $this->jsonRPCClient->execute("verifymessage", array($address, $signature, $message));
    }
    ...
    {
        return $this->jsonRPCClient->execute("publish", array($streamID, $key, $dataHex));
    }


    public function publishFrom($fromAddress, $streamID, $key, $dataHex)
    {
        return $this->jsonRPCClient->execute("publishfrom", array($fromAddress, $streamID, $key, $dataHex));
    }
